"""
TP de séries temporelles (Master 2) : comparaison ARIMA, ARIMA automatique et
modèle de type Prophet sur un jeu de données libre de droit.

Ce script illustre trois approches classiques de prévision sur une série
temporelle réelle :
...
1. **ARIMA manuel** : on choisit les paramètres (p, d, q) à partir d'une
   ...
   et on choisit p et q après avoir examiné l'autocorrélation (voir le
   notebook ou le cours). Le script utilise l'ordre (2, 1, 3) qui se révèle
...
3. **Modèle de type Prophet** : si la librairie `prophet` (anciennement
   `fbprophet`) est installée dans votre environnement, ce script ajuste un
   modèle Prophet de Meta. Prophet décompose la série en tendance,
   saisonnalité et jours spéciaux. Le jeu de données ne contient pas de
   jours fériés spécifiques, mais Prophet s'adapte bien à la tendance
   croissante et à la saisonnalité annuelle. Si `prophet` n'est pas
   disponible, le script crée une prédiction de base en utilisant une
   régression linéaire avec composantes sinusoïdales (tendance + saison)
   afin d'illustrer un modèle additif comparable (cette section sera
   indiquée comme « placeholder Prophet »).
...
* divise la série en une partie d'entraînement et une partie de test
  ...
  hebdomadaires) ;
* ajuste le modèle sur l'entraînement, génère des prévisions sur le jeu de
...
fournit par `statsmodels`. Il s'agit de mesures hebdomadaires de la
...
def fit_prophet_model(train: pd.Series):
    """Ajuste un modèle Prophet sur la série d'entraînement.

    Cette fonction requiert l'installation de la librairie `prophet` (anciennement
    `fbprophet`). Si `prophet` n'est pas disponible, une ImportError est levée.
    ...
    """
    # Import local pour éviter de dépendre de Prophet si non installé
    try:
        from prophet import Prophet
    except Exception as e:
        raise ImportError(
            "La librairie 'prophet' n'est pas installée. Veuillez l'installer via pip (pip install prophet)"
        ) from e
...
def predict_prophet(model, horizon_index: pd.DatetimeIndex) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Génère des prévisions et intervalles de confiance pour un modèle Prophet.
    ...
    horizon_index : pd.DatetimeIndex
        Index des dates futures pour lesquelles générer les prévisions.
    ...
    tuple[np.ndarray, np.ndarray, np.ndarray]
        yhat, lower, upper
    """
    from prophet import Prophet  # import à l'intérieur pour éviter dépendance globale
    future = pd.DataFrame({'ds': horizon_index})
...
def main():
    ...
    Ce flux illustre trois approches pédagogiques de prévision :
      ...
      4. **Modèle Prophet de Meta :** si la librairie `prophet` est
         disponible, on ajuste un modèle Prophet (tendance + saisonnalité) et
         on génère des prévisions. Si `prophet` n'est pas installé, on
         informe l'utilisateur.

      5. **Évaluation et visualisation :** on calcule les métriques RMSE, MAE
    ...
    # 5. Ajustement du modèle Prophet (vrai modèle)
    print("\nAjustement du modèle Prophet...")
    try:
        prophet_model = fit_prophet_model(train)
        prophet_pred, prophet_lower, prophet_upper = predict_prophet(prophet_model, test.index)
        model_name = 'Prophet'
    except Exception as e:
        prophet_pred = None
        prophet_lower = prophet_upper = None
        model_name = None
        # On informe l'utilisateur que Prophet n'est pas disponible
        print("La librairie 'prophet' n'est pas installée ou a rencontré un problème:\n", e)
    ...
    if prophet_pred is not None:
        results.append({'Model': model_name, **evaluate_forecast(test.values, prophet_pred)})
    ...
    # Prévisions Prophet et zones, si disponible
    if prophet_pred is not None:
        plt.plot(test.index, prophet_pred, label=model_name)
        if not np.array_equal(prophet_lower, prophet_pred):
            plt.fill_between(test.index, prophet_lower, prophet_upper, alpha=0.2)
